...
def sitka(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `sitka.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 395 rows and 4 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "sitka.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/MASS/Sitka.csv"
