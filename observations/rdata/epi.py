...
def epi(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `epi.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 3570 rows and 57 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "epi.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/psych/epi.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="epi.csv",
			       resume=False)
