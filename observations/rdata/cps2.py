...
def cps2(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `cps2.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 2369 rows and 10 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "cps2.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/DAAG/cps2.csv"
