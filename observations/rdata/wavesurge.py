...
def wavesurge(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `wavesurge.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 2894 rows and 2 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "wavesurge.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/texmex/wavesurge.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="wavesurge.csv",
			       resume=False)
