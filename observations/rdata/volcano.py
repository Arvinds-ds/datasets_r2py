...
def volcano(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `volcano.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 87 rows and 61 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "volcano.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/datasets/volcano.csv"
