...
def minard_troops(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `minard_troops.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 51 rows and 5 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "minard_troops.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/HistData/Minard.troops.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="minard_troops.csv",
			       resume=False)
