...
def weight_loss_incentive4(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `weight_loss_incentive4.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 36 rows and 2 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "weight_loss_incentive4.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/Stat2Data/WeightLossIncentive4.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="weight_loss_incentive4.csv",
			       resume=False)
