...
def pima_te(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `pima_te.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 332 rows and 8 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "pima_te.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/MASS/Pima.te.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="pima_te.csv",
			       resume=False)
