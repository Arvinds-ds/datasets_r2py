...
def bigcity(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `bigcity.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 49 rows and 2 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "bigcity.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/boot/bigcity.csv"
