...
def cps3(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `cps3.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 429 rows and 10 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "cps3.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/DAAG/cps3.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="cps3.csv",
			       resume=False)
