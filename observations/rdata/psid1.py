...
def psid1(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `psid1.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 2490 rows and 10 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "psid1.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/DAAG/psid1.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="psid1.csv",
			       resume=False)
