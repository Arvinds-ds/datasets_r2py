...
def bechtoldt_2(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `bechtoldt_2.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 17 rows and 17 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "bechtoldt_2.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/psych/Bechtoldt.2.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="bechtoldt_2.csv",
			       resume=False)
