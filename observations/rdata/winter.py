...
def winter(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `winter.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 532 rows and 5 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "winter.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/texmex/winter.csv"
