...
def incidents_by_country_yr(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `incidents_by_country_yr.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 206 rows and 45 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "incidents_by_country_yr.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/Ecdat/incidents.byCountryYr.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="incidents_by_country_yr.csv",
			       resume=False)
