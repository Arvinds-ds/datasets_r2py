...
def pima_tr2(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `pima_tr2.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 300 rows and 8 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "pima_tr2.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/MASS/Pima.tr2.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="pima_tr2.csv",
			       resume=False)
