...
def bomregions2012(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `bomregions2012.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 113 rows and 22 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "bomregions2012.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/DAAG/bomregions2012.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="bomregions2012.csv",
			       resume=False)
