...
def snow_pumps(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `snow_pumps.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 13 rows and 4 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "snow_pumps.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/HistData/Snow.pumps.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="snow_pumps.csv",
			       resume=False)
