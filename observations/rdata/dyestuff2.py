...
def dyestuff2(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `dyestuff2.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 30 rows and 2 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "dyestuff2.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/lme4/Dyestuff2.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="dyestuff2.csv",
			       resume=False)
