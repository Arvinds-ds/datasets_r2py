...
def langren1644(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `langren1644.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 12 rows and 9 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "langren1644.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/HistData/Langren1644.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="langren1644.csv",
			       resume=False)
