...
def snow_deaths2(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `snow_deaths2.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 578 rows and 3 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "snow_deaths2.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/HistData/Snow.deaths2.csv"
    maybe_download_and_extract(path, url,
			       save_file_name="snow_deaths2.csv",
			       resume=False)
