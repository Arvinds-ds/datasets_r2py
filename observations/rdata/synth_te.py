...
def synth_te(path):
  ...
  Args:
    path: str.
      ...
      be downloaded and extracted there. Filename is `synth_te.csv`.
  Returns:
    Tuple of np.ndarray `x_train` with 1000 rows and 3 columns and
    dictionary `metadata` of column headers (feature names).
  """
  path = os.path.expanduser(path)
  filename = "synth_te.csv"
  if not os.path.exists(os.path.join(path, filename)):
    url = "https://raw.github.com/vincentarelbundock/Rdatasets/master/csv/MASS/synth.te.csv"
